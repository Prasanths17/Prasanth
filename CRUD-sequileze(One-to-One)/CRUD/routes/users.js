...
router.get('/employeeInfo' , (req,res) => {
  pool.query(`select * from employeeInfo as ei join employeeSalary as es on ei.id=es.id` , (err , result) => {
      if(err){
          console.log(err)
...
router.post('/employeeInfo' , async (req,res) => {
  ...
  pool.query(`insert into employeeInfo (name,Address,job_title) values (?,?,?)`, [name,Address,job_title] , (err,result) => {    
      ...
      pool.query(`insert into employeeSalary (salary) values (?)`,[salary] , (err,result) => {
          if(err){
              console.log(err);
              res.status(500).send(`Error in creating`);
          }else{
